...
const App: React.FC = () => {
  ...
  const handleInitiate = async (data: BirthData) => {
    ...
    try {
      ...
      const finalizedResult: OracleResult = {
        ...
        culture: briefData.culture || matchedArchetype.culture
      };
    ...
    return (
      <ObsidianContainer>
        <div className="flex items-center justify-center min-h-[60vh]">
          <p className="text-gray-600 text-[10px] tracking-widest uppercase">Consulting the void...</p>
        </div>
      </ObsidianContainer>
    );
  }
  ...
  return (
    <ObsidianContainer>
      {/* Sign-out — only visible when authenticated and not mid-ritual */}
      {user && appState === AppState.IDLE && (
        <div className="absolute top-4 right-6 z-20">
          <button
            onClick={handleSignOut}
            className="text-gray-600 text-[9px] tracking-widest uppercase hover:text-gray-400 transition-colors"
          >
            Exit
          </button>
        </div>
      ...
      {user && appState === AppState.IDLE && <InputForm onSubmit={handleInitiate} />}

      {user && [AppState.ANALYZING, AppState.COMMUNING, AppState.MANIFESTING].includes(appState) && (
        <LoadingOracle state={appState} />
      )}

      {user && appState === AppState.REVEALED && result && (
        <ResultReveal result={result} onReset={reset} />
      )}

      {user && appState === AppState.ERROR && (
